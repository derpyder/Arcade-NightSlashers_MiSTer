...
module jtframe_obj_buffer_fix #(parameter
    DW          = 8,
    AW          = 9,
    ALPHAW      = 4,
    ALPHA       = 32'HF,
    ...
    input   we,
    input   draw_busy,          // FIX: obj draw FSM busy; hold the ping-pong swap until draw idle
...
reg     line, last_LHBL, new_we, swap_pend;
...
// FIX (fill-vs-swap race): the stock buffer toggles `line` UNCONDITIONALLY on LHBL-fall,
// so a heavy line whose draw over-runs into hblank gets its half swapped to display mid-write
// (the per-scanline "comb"). Here we LATCH the swap request on LHBL-fall and only apply the
// toggle once the obj draw FSM is idle (draw_busy==0). The swap is deferred, never dropped,
// so under normal load it is bit-identical to the stock behaviour.
always @(posedge clk) begin
    last_LHBL <= LHBL;
    if( !LHBL && last_LHBL )
        swap_pend <= 1'b1;              // request swap at hblank entry
    if( swap_pend && !draw_busy ) begin // apply only when the fill FSM has finished this line
        line      <= ~line;
        swap_pend <= 1'b0;
    end
end
