...
// Cycle-by-cycle PROBE of the obj0 FSM <-> real DW32-DOUBLE OKLATCH=1 cache at REAL addresses,
// driven with the FAITHFUL jtnslasher_obj cadence (2nd-ok consume, back-to-back).  Dumps the seam
// internals (FSM state, obj0_ok level, bcache hit0/hit1/double/dend/cache_ok/good) so we can SEE
// whether the OKLATCH stale-ok / DOUBLE-flip behaviour at real addresses matches expectation, and
// whether the plane4 read is a cache HIT or an unexpected MISS.
module tb_objfold_real_probe;
    ...
    // hierarchical handles into the bcache
    `define BC u_bank3.u_2slots.u_slot0.genblk1.u_block_cache

    reg probe=0; integer cyc=0;
    always @(posedge clk) begin
        cyc<=cyc+1;
        if( probe )
            $display("  cyc=%0d st=%0d e_cs=%b f_cs=%b f_ok=%b f_addr=%06x f_data=%08x | h0=%b h1=%b dbl=%b dend=%b cok=%b good=%b req=%b | rom_ok=%b rom_data=%010x",
                cyc, u_dut.o0st, obj0_rom_cs, obj0_cs, obj0_ok, obj0_addr, obj0_data,
                `BC.hit0, `BC.hit1, `BC.double, `BC.dend, `BC.cache_ok, `BC.good, `BC.req,
                obj0_rom_ok, obj0_rom_data);
    end

    // faithful single-tile fetch (2nd-ok consume)
    reg rom_good;
    task fetch_tile(input integer i);
        integer t;
        begin
            $display("--- tile i=%0d engaddr=%06x nwi=0x%0x golden=%010x ---",
                     i, tv_addr[i][20:0], {tv_addr[i][20:5],~tv_addr[i][0],tv_addr[i][4:1]}, tv_gold[i]);
            @(posedge clk); #1; obj0_rom_addr=tv_addr[i][20:0]; obj0_rom_cs=1; rom_good=0; probe=1;
            t=0;
            // consume on 2nd consecutive ok
            forever begin
                @(posedge clk); #1;
                if( obj0_rom_cs && rom_good && obj0_rom_ok ) begin
                    $display("  CONSUME got=%010x gold=%010x -> %s",
                             obj0_rom_data, tv_gold[i], obj0_rom_data===tv_gold[i]?"MATCH":"*** MISMATCH ***");
                    obj0_rom_cs=0;
                    t=999;
                end
                rom_good = obj0_rom_cs & obj0_rom_ok;  // registered next-cycle value
                t=t+1;
                if( t>=400 ) begin $display("  TIMEOUT"); obj0_rom_cs=0; end
                if( !obj0_rom_cs ) begin @(posedge clk); probe=0; disable fetch_tile; end
            end
        end
    endtask
    ...
    initial begin
        ...
        for( idx=0; idx<`OBJFOLD_N; idx=idx+1 ) if(fi<0 && tv_gold[idx]!==40'd0) fi=idx;
        $display("=== REAL-addr OKLATCH seam probe: first nonzero tile index=%0d ===", fi);
        fetch_tile(fi);        // cold
        fetch_tile(fi);        // warm (re-read same tile = both halves cache hits)
        fetch_tile(fi+1);      // adjacent half (the half-0/half-1 pair partner)
        fetch_tile(fi+2);
        // back-to-back with NO gap: drive fi then immediately fi+1 (already covered by the engine in
        // tb_objfold_real; here we just show the per-tile seam)
        $finish;
    end
