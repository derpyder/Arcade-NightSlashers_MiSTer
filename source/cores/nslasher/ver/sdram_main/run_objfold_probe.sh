...
python3 gen_objfold_combined.py >/dev/null
for f in deco56_address.hex deco56_xor.hex deco56_swap.hex deco74_address.hex deco74_xor.hex deco74_swap.hex; do
    [ -e "$f" ] || cp "$GFX/$f" .
done
iverilog -g2012 -I . -I "$CORE" -I "$GFX" -o tb_objfold_probe.vvp \
  tb_objfold_probe.v $CORE/jtnslasher_sdram.v $CORE/jtnslasher_gfxdec.v \
