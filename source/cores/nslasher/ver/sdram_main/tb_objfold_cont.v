...
module tb_objfold_cont;
    ...
    // ===== CONTENTION MODEL (the Tempest lesson): competing BA1/BA2 masters + refresh =====
    // On HW, BA1 (ARM/Z80/OKI) + BA2 (tilemaps/obj1) hammer the shared controller every cycle and
    // refresh stalls it periodically -> obj0_ok (BA3) arrives LATE and IRREGULAR. The original sim
    // had rfsh=0 and ba_rd[2:0]=0 (zero contention). Here we add faithful competing read traffic.
    reg  [SDRAMW-1:0] ba1_a=0, ba2_a=0;
    reg              ba1_req=0, ba2_req=0;
    reg  [1:0]       ba1_st=0,  ba2_st=0;
    wire [3:0]  ba_rd = {rom_rd, ba2_req, ba1_req, 1'b0};
    wire [3:0]  ba_wr = 4'd0;
    // throttled competing masters: REQUEST -> ACK -> RDY -> IDLE(GAP) -> repeat. GAP = duty knob.
    integer GAP; initial if(!$value$plusargs("gap=%d",GAP)) GAP=6;
    reg [7:0] ba1_g=0, ba2_g=0; reg ba1_en=1, ba2_en=1;
    initial begin if(!$value$plusargs("ba1=%d",ba1_en)) ba1_en=1; if(!$value$plusargs("ba2=%d",ba2_en)) ba2_en=1; end
    always @(posedge clk) if(rst) begin ba1_req<=0; ba1_a<=0; ba1_st<=0; ba1_g<=0; end
        else if(ba1_en) case(ba1_st)
            0: begin ba1_req<=1; ba1_st<=1; end
            1: if(ba_ack[1]) begin ba1_req<=0; ba1_st<=2; end
            2: if(ba_rdy[1]) begin ba1_a<=ba1_a+23'h401; ba1_g<=GAP[7:0]; ba1_st<=3; end
            3: if(ba1_g==0) ba1_st<=0; else ba1_g<=ba1_g-8'd1;
            default: ba1_st<=0;
        endcase
    always @(posedge clk) if(rst) begin ba2_req<=0; ba2_a<=0; ba2_st<=0; ba2_g<=0; end
        else if(ba2_en) case(ba2_st)
            0: begin ba2_req<=1; ba2_st<=1; end
            1: if(ba_ack[2]) begin ba2_req<=0; ba2_st<=2; end
            2: if(ba_rdy[2]) begin ba2_a<=ba2_a+23'h807; ba2_g<=GAP[7:0]; ba2_st<=3; end
            3: if(ba2_g==0) ba2_st<=0; else ba2_g<=ba2_g-8'd1;
            default: ba2_st<=0;
        endcase
    // refresh generator: pulse rfsh periodically (~64us @48MHz ~ 3072 clk; use 1500 = aggressive)
    reg [11:0] rfsh_cnt=0; reg rfsh_pulse=0;
    always @(posedge clk) if(rst) begin rfsh_cnt<=0; rfsh_pulse<=0; end
        else begin rfsh_pulse<=0; rfsh_cnt<=rfsh_cnt+1'b1; if(rfsh_cnt==12'd3000) begin rfsh_cnt<=0; rfsh_pulse<=1; end end
    ...
    jtframe_sdram64 #(
        ...
        .ba0_addr({SDRAMW{1'b0}}), .ba1_addr(ba1_a), .ba2_addr(ba2_a), .ba3_addr(rom_saddr),
        .rd(ba_rd), .wr(ba_wr),
