#!/bin/bash
# Confirm the refresh-stressed faithful cont sim is ALSO a truthful discriminator (BUGGY FSM -> RED).
set -e
cd "$(dirname "$0")"
...
iverilog -g2012 -DSIMULATION -DJTFRAME_SIM_SDRAM_NONSTOP -DJTFRAME_SIM_ROMRQ_NOCHECK \
  -I . -I "$CORE" -I "$GFX" -o tb_objfold_real_cont_buggy.vvp \
  tb_objfold_real_cont.v jtnslasher_sdram_buggyrenamed.v $CORE/jtnslasher_gfxdec.v \
...
vvp tb_objfold_real_cont_buggy.vvp
