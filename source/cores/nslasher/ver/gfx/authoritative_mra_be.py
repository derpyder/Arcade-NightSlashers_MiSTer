...
def bank_words(start, end): return [(blob[i]<<8) | blob[i+1] for i in range(start, min(end,len(blob)), 2)]  # BIG-ENDIAN load
...
# ---- dump the BE SDRAM bank3 (obj0) image for an RTL-level re-confirmation ----
import sys as _sys
if "--dump" in _sys.argv:
    with open("sdram_bank3_be.hex","w") as f:
        prev=None
        for a in sorted(sd3):
            if a!=prev: f.write("@%X
"%a)
            f.write("%04X
"%(sd3[a]&0xffff)); prev=a+1
    print("dumped sdram_bank3_be.hex (%d words)"%len(sd3))
